...
class Bilinear(nn.Module):
    def __init__(self, n_in, n_out, np):
        super().__init__()
        scale_fac = (n_in*(np+1))**0.5*10
        self.Alin = nn.Parameter(torch.randn((n_out, n_in))/scale_fac)
        self.Anlin = nn.Parameter(torch.randn((np, n_out, n_in))/scale_fac)
    
    def forward(self, p):
        #p (Nb, np) 
        #Anlin (np, n_out, n_in) -> (1, np, n_out, n_in)
        #self.Alin (n_out, n_int) -> (None, n_out, n_in)
        return self.Alin[None] + (self.Anlin[None]*p[:,:,None,None]).sum(1)
